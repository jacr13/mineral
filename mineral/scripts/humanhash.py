"""ORIGINAL FROM: https://github.com/blag/humanhash/tree/master.
...
if sys.version_info.major == 3:  # pragma: nocover
    ...
    def map(*args, **kwargs):
        return list(py3_map(*args, **kwargs))
...
class HumanHasher(object):
    ...
    def __init__(self, wordlist=DEFAULT_WORDLIST):
        """Initialize the HumanHasher with a given wordlist.
    ...
    def humanize_list(self, hexdigest, words=4):
        ...
        bytes_ = map(lambda x: int(x, 16), map("".join, zip(hexdigest[::2], hexdigest[1::2], strict=False)))
    ...
    def uuid(self, **params):
        r"""Generate a UUID with a human-readable representation.
